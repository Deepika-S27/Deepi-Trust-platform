...
const AdminDashboard = () => {
  ...
  const {
    ...
    getAdminQR, setAdminQR, getCenters, getAgents, updateAgentFee
  } = useAppContext();
                    ...
                    <div key={order.id} className="task-card" style={{ position: 'relative', overflow: 'hidden' }}>
                      {scanningId === order.id && (
                        <div className="scan-overlay">
                          ...
                          <p style={{ fontWeight: 600, color: 'var(--primary)' }}>AI Scanning...</p>
                          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Analyzing freshness & safety</p>
                        </div>
                      )}
                    ...
                    <div key={order.id} className="task-card">
                      ...
                      {order.ai_details && (
                        <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: 'rgba(45,106,79,0.04)', borderRadius: 'var(--radius-md)' }}>
                          {order.ai_details.map((d, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem', fontSize: '0.82rem' }}>
                              ...
                              <div style={{ width: '50px', height: '4px', background: 'rgba(0,0,0,0.08)', borderRadius: '10px', overflow: 'hidden' }}>
                                <div style={{ width: `${d.score}%`, height: '100%', background: d.status === 'pass' ? 'var(--success)' : 'var(--warning)', borderRadius: '10px' }} />
                              </div>
                              <span style={{ fontWeight: 600, fontSize: '0.8rem', width: '30px', textAlign: 'right', color: d.status === 'pass' ? 'var(--success)' : 'var(--warning)' }}>{d.score}%</span>
                            </div>
                          ))}
                        </div>
                      )}
